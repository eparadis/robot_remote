...
while True:
    ...
    ### computer vision
    # grab an image from the remote camera
    urllib.urlretrieve("http://192.168.1.143:8080/shot.jpg", "image.jpg")
    src_image = cv2.imread("image.jpg")
    cv2.imshow("input image", src_image)
    # search for markers
    aruco = '/Users/ed/src/opencv/build/aruco-1.2.4/build/utils/aruco_simple'
    image = "image.jpg"
    cal = '/Users/ed/git/robot_remote/htc_one_cal/cal.yml'
    size = '1.75' # side of marker in inches, only counting the black part, which might be wrong?
